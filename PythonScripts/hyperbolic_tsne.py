...
def _gradient_descent(objective, p0, it, n_iter, objective_error=None,
    ...
    for i in range(it, n_iter):
        ...
        if HYPERBOLIC:
            ...
            lengths = np.linalg.norm(X_embed, axis=1)
            X_embed /= np.maximum(1, lengths[:, np.newaxis]/(1-1e-5))
            p = X_embed.flatten()
...
def test2():
    global HYPERBOLIC
    HYPERBOLIC = False
    X = datasets.fetch_20newsgroups_vectorized(data_home='.').data[:1000]
    print(X.shape)
    n_components = 2
    perplexity = 400
    tsne = TSNE(n_components=n_components, perplexity=perplexity, method='exact')
...
def hypertsne_plotting(embeddings,objects):
    HYPERBOLIC = True
    ...
    plt.scatter(Y[:, 0], Y[:, 1], c="b")
    # not nice but prob doable plot all likes after now
    # for edge in edges:
    #     child, parent = edge
    #     child_idx = objects.index(child)
    #     parent_idx = objects.index(parent)
        
    #     child_node = Y[child_idx]
    #     parent_node = Y[parent_idx]
        
    #     x_cdt = [child_node[0],parent_node[0]]
    #     y_cdt = [child_node[1],parent_node[1]]
        
    #     plt.plot(x_cdt,y_cdt)
        
    
    #then plot labels
    for label, x, y in zip(objects, Y[:, 0], Y[:, 1]):
        plt.annotate(label, xy=(x, y), xytext=(0, 0), textcoords='offset points')
    plt.show()
    
def hypertsne(embeddings, objects):
    HYPERBOLIC = True
    
    print('Hyper TSNE has been called.')
...
def plot(Y,objects,edges):
    HYPERBOLIC = True
    
    
    plt.subplots(figsize=(50,50))
    plt.suptitle("hyperbolic plot")
    plt.scatter(Y[:, 0], Y[:, 1], c="b")
    # not nice but prob doable plot all likes after now
    for edge in edges:
        child, parent = edge
